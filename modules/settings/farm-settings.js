...
export async function mountFarmSettings(container, onSave) {
  ...
  // Fetch live site list via RPC to get distinct sites without row limit issues
  let liveSites = [];
  try {
    const rows = await dbSelect('rpc/get_cc_sites', '');
    liveSites = rows.map(r => r.site).filter(Boolean).sort();
    console.log('[farm-settings] Live CC sites loaded:', liveSites.length,
      liveSites.includes('Emerald Hill') ? '✓ Emerald Hill found' : '✗ Emerald Hill MISSING');
  } catch(e) {
...
export async function mountFarmSettings(container, onSave) {
  ...
  // Use Range header to bypass PostgREST default 1000-row limit
  let liveSites = [];
  try {
    const yesterday = new Date(Date.now() - 86400000).toISOString().split('T')[0];
    const SUPABASE_URL = (await import('../../js/supabase-client.js')).SUPABASE_URL ||
      'https://nqvfuqvindsgnogejaei.supabase.co';

    // Fetch all distinct regions in one call using Range: 0-4999
    const { getSession } = await import('../../js/app-state.js');
    const session = getSession();
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/market_prices?select=region&source_label=eq.CropConnect&price_date=gte.${yesterday}&order=region`,
      {
        headers: {
          'apikey': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im5xdmZ1cXZpbmRzZ25vZ2VqYWVpIiwicm9sZSI6ImFub24iLCJpYXQiOjE2OTk0MDk5NjUsImV4cCI6MjAxNDk4NTk2NX0.FdPY5AABV4UuDDfNgBOOUhJJQXSLEJit0DJzQVeP-Rc',

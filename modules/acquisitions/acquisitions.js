...
const MGMT_STATUSES = ['CFM available to manage', 'Outside scope', 'CFM already manages'];
...
const MGMT_COLOURS = {
  ...
  'Outside scope':           { bg: '#f3f4f6', color: '#6b7280' },
...
function _renderPipeline(content, container) {
  ...
  const notInterestedDeals = _deals.filter(d => (d.cfm_management_status === 'Outside scope' || d.cfm_management_status === 'CFM not interested') && d.status !== 'Sold');
  ...
  content.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:16px">
      ${[
        ...
        ['Available to manage', _deals.filter(d=>d.cfm_management_status==='CFM available to manage'||d.cfm_management_status==='Available'||d.cfm_management_status==='CFM already manages').length, '#3B6D11'],
...
function _renderList(content, container) {
  let deals = _deals.filter(d => d.status !== 'Sold' && d.cfm_management_status !== 'CFM not interested' && d.cfm_management_status !== 'Outside scope');
  ...
  if (_filterMgmt) deals = deals.filter(d => d.cfm_management_status === _filterMgmt || 
    (_filterMgmt === 'Outside scope' && d.cfm_management_status === 'CFM not interested'));
        ...
        <div style="display:flex;flex-direction:column;gap:0">
          <div class="form-group">
            ...
            <select class="form-select" id="d-status">
              ${STATUSES.map(s => `<option value="${s}" ${(existing?.status||'Reviewing')===s?'selected':''}>${s}</option>`).join('')}
            </select>
          </div>
          <div class="form-group">
          ...
          <!-- CFM Services -->
          <div style="padding:10px 12px;background:var(--page-bg);border-radius:6px;margin-bottom:8px">
            <p style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:var(--hint);margin-bottom:10px">CFM services</p>
            <div class="form-group">
            ...
            <div style="display:flex;gap:16px;align-items:flex-end">
              <div>
                <label style="font-size:11px;color:var(--hint);display:block;margin-bottom:3px">🌿 Agronomy</label>
                <select class="form-select" id="d-agronomy" style="width:140px">
                  <option value="">None</option>
                  <option value="Broadacre" ${existing?.agronomy_service==='Broadacre'?'selected':''}>Broadacre</option>
                  <option value="Horticulture" ${existing?.agronomy_service==='Horticulture'?'selected':''}>Horticulture</option>
                  <option value="Both" ${existing?.agronomy_service==='Both'?'selected':''}>Both</option>
                </select>
              </div>
              <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;padding-bottom:6px">
                <input type="checkbox" id="d-hr" ${existing?.hr_service?'checked':''} style="width:16px;height:16px;cursor:pointer">
                👤 HR
              </label>
            </div>
          </div>

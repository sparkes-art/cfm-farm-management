...
function _renderTable(container) {
  ...
  const cropRows = rows.filter(i => i.master_unit !== 'head');
  const lsRows   = rows.filter(i => i.master_unit === 'head');
  const totalGross   = cropRows.reduce((s,i) => s + (parseFloat(i.gross_amount)||0), 0);
  const totalQA      = cropRows.reduce((s,i) => s + (parseFloat(i.total_quality_adj)||0), 0);
  const totalIncome  = totalGross + totalQA;
  const totalLsHead  = lsRows.reduce((s,i) => s + (parseFloat(i.total_qty)||0), 0);
  const totalLsGross = lsRows.reduce((s,i) => s + (parseFloat(i.gross_amount)||0), 0);
  ...
  wrap.innerHTML = `
    <div style="display:flex;gap:0;flex-wrap:wrap;border-bottom:1px solid var(--border-light)">
      ${cropRows.length ? `
      <div style="padding:12px 18px;border-right:1px solid var(--border-light)">
        ...
        <div style="font-size:18px;font-weight:700;color:var(--ink)">${formatCurrency(totalGross, 0)}</div>
      </div>
      <div style="padding:12px 18px;border-right:1px solid var(--border-light)">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:.07em;color:var(--hint);margin-bottom:4px">Quality adj</div>
        <div style="font-size:18px;font-weight:700;color:${totalQA>=0?'var(--green)':'var(--red)'}">${totalQA?(totalQA>0?'+':'')+formatCurrency(totalQA,0):'—'}</div>
      </div>
      <div style="padding:12px 18px;border-right:1px solid var(--border-light)">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:.07em;color:var(--hint);margin-bottom:4px">Total income</div>
        <div style="font-size:18px;font-weight:700;color:var(--blue)">${formatCurrency(totalIncome, 0)}</div>
      </div>` : ''}
      ${lsRows.length ? `
      <div style="padding:12px 18px;border-right:1px solid var(--border-light)">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:.07em;color:var(--hint);margin-bottom:4px">🐄 Head sold</div>
        <div style="font-size:18px;font-weight:700;color:var(--ink)">${formatNumber(totalLsHead, 0)}</div>
      </div>
      <div style="padding:12px 18px;border-right:1px solid var(--border-light)">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:.07em;color:var(--hint);margin-bottom:4px">🐄 Livestock gross</div>
        <div style="font-size:18px;font-weight:700;color:var(--green)">${formatCurrency(totalLsGross, 0)}</div>
      </div>` : ''}
      <div style="padding:12px 18px;border-right:1px solid var(--border-light)">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:.07em;color:var(--hint);margin-bottom:4px">Invoices</div>
...
// ── Livestock sale form ────────────────────────────────────────
export async function openLivestockForm(container, existing = null) {
  const farm = getActiveFarm();
  const isEdit = !!existing;

  // Load livestock mobs for mob picker
  let stockItems = [];
  try {
    stockItems = await dbSelect('stock_items',
      `farm_id=eq.${farm.id}&category=eq.livestock&active=eq.true&order=subgroup,name`
    );
  ...
  modal.querySelector('#ls-save').addEventListener('click', async () => {
    ...
    try {
      ...
      if (existing?.id) {
        ...
        savedInvoiceId = saved?.[0]?.id || saved?.id;
